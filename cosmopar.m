function varargout = cosmopar(what,varargin)
%hello world
...
%                         resp = D.tuning(n,t);
                        ...
                        sptemp = LIFModel(spkInds,spkVec,gSharedVec,gIndep,gAnat,anatSign,dT,stimDur,plotOn);
                        spikes{n,1} = sptemp;
                        spikeNum(n)  = numel(sptemp);
                        neuron(n)    = n;
                        numRun(n)    = r;
                        numRep(n)    = rep;
                        prefDir(n)   = D.prefDir(n);
                        stimDir(n)   = stims(t);
                        anatSignv(n)  = anatSign;
                    end
                    T.spikes = spikes;
                    T.spikeNum = spikeNum;
                    T.neuron = neuron;
...
%         load(fullfile(dataDir,'T1_temp'));
        ...
        stimDirt1 = T1.stimDir;
        spikeNum_meant1 = T1.spikeNum_mean;
...
%         load(fullfile(dataDir,'T2_temp'));
        ...
        stimDirt2 = T2.stimDir;
        spikeNum_meant2 = T2.spikeNum_mean;
        spikeNum_vart2 = T2.spikeNum_var;
        
        for i=1:numNeuron
            parfor j=i:numNeuron
                corrMean(j)      = corr(spikeNum_meant1(neuront1==i),spikeNum_meant1(neuront1==j));
                corrVar(j)       = corr(spikeNum_vart2(neuront2==i),spikeNum_vart2(neuront2==j));
            end
            for j=i:numNeuron
                neuron1(j)       = i;
                neuron2(j)       = j;
                sameNeuron(j)    = double(i==j);
                pref1           = prefDirt1(neuront1==i);
                pref2           = prefDirt1(neuront1==j);
                anat1           = anatSign(neuron==i);
                anat2           = anatSign(neuron==i);
                prefDir1(j)      = pref1(1);
                prefDir2(j)      = pref2(1);
                prefSame(j)      = double(prefDir1(end)==prefDir2(end));
                anatSign1(j)     = anat1(1);
                anatSign2(j,:)     = anat2(1);
            end
            D.corrMean      = corrMean;
