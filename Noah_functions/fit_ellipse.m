function ellipse_t = fit_ellipse( x,y,axis_handle, colors )
%
...
if (nargin>2) & ~isempty( axis_handle ) & (test>0)
    ...
    %plot( new_ver_line(1,:),new_ver_line(2,:),'Color', colors, 'LineWidth', 2 );
    %plot( new_horz_line(1,:),new_horz_line(2,:),'Color', colors, 'LineWidth', 2 );
    foo = plot( rotated_ellipse(1,:),rotated_ellipse(2,:),'Color', colors, 'LineWidth', 2 );
    foo.Color(4) = .25;
    p = patch(rotated_ellipse(1,:),rotated_ellipse(2,:), colors);
    p.FaceAlpha=.25;
    p.EdgeColor = 'none';
